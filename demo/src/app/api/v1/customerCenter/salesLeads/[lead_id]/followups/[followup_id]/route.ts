...
type RouteContext = { params: Promise<{ lead_id: string; followup_id: string }> };
...
export async function GET(req: NextRequest, context: RouteContext) {
  ...
  const { lead_id, followup_id } = await context.params;
  return proxyToBackend(
    `/api/v1/customerCenter/salesLeads/${lead_id}/followups/${followup_id}`,
    req,
    {
      extraHeaders: {
